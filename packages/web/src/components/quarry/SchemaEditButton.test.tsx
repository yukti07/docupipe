...
describe("SchemaEditButton", () => {
  it.each([
    ...
    render(<SchemaEditButton state={state} onOpen={vi.fn()} />)
    expect(screen.getByRole("button", { name: label })).toBeDisabled()
  })

  it("opens once the schema is in, and counts the tables behind it", async () => {
    const onOpen = vi.fn()
    const { user } = render(<SchemaEditButton state="ready" tableCount={3} onOpen={onOpen} />)
  ...
  it("holds water at half while it waits, and fills when the schema lands", () => {
    const { container, rerender } = render(<SchemaEditButton state="loading" onOpen={vi.fn()} />)
    ...
    rerender(<SchemaEditButton state="ready" tableCount={1} onOpen={vi.fn()} />)
  ...
  it("shows no water at all before the file has landed", () => {
    const { container } = render(<SchemaEditButton state="uploading" onOpen={vi.fn()} />)
